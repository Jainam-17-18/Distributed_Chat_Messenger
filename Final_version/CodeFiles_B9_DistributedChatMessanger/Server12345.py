...
# Create a table to store online user's username and timestamp
c.execute('''CREATE TABLE IF NOT EXISTS online_users
             (username TEXT PRIMARY KEY, timestamp TEXT)''')
...
def handle_client(conn, addr):
    ...
    c.execute("INSERT INTO online_users (username, timestamp) VALUES (?, ?)", (username, now))
    ...
    try:
        while True:
            ...
            if data == 'online':  # check if the message is "online"
                c.execute("SELECT username FROM online_users WHERE username!=?", (username,))
                result = c.fetchall()
                ...
                if recipient in clients:
                    ...
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s1:
                        s1.connect(('localhost', 9999))  # connect to other server
